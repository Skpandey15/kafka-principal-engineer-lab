...
/**
 * A small, direct wrapper around Kafka Connect's REST API
 * ({@code POST /connectors}, {@code GET /connectors/{name}/status},
 * {@code DELETE /connectors/{name}}) using the JDK's own
 * {@link HttpClient} -- there is no dedicated "Kafka Connect Java client"
 * library the way there is for the broker (kafka-clients) or Schema
 * Registry (kafka-schema-registry-client, WP-10); the REST API IS the
 * client interface, by design, exactly as `docs/kafka-connect/` explains.
 */
public final class ConnectRestClient {
    ...
    public JsonNode connectorPlugins() throws Exception {
        HttpRequest request = HttpRequest.newBuilder(URI.create(baseUrl + "/connector-plugins"))
                .timeout(Duration.ofSeconds(10))
                .GET()
                .build();
        HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
