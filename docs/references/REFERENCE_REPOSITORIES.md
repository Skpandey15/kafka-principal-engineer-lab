...
| Target WP(s) | All of them, in some form — this is the repository every other reference is checked against. Most directly: WP-03 (Java client), WP-05 through WP-08 (consumer groups, replication, KRaft, transactions), and the source-reading track (WP-24). |
...
| Target WP(s) | WP-14 (`lab-18-spring-kafka`), and cross-referenced from WP-15 (retry/DLQ) since Spring Kafka's non-blocking retry topics are a framework-level implementation of the same pattern taught natively first. |
...
| What NOT to copy | Do not treat CDC as a substitute for understanding Kafka Connect's own worker/task/offset model first — WP-10 (plain Kafka Connect) is sequenced before WP-11 (Debezium specifically) for that reason. |
...
| Target WP(s) | WP-04 (partitioning), WP-13 (Streams), as a pattern cross-check only. |
...
| Target WP(s) | WP-13 (`lab-17-kafka-streams`), pattern reference only. |
...
| Target WP(s) | The future idempotent-consumer curriculum topic (conceptually anchored after WP-12, hands-on lab delivered in WP-15 alongside retry/DLQ — see the curriculum map). |
