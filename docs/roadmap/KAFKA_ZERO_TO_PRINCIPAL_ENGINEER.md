...
| WP-13 | Retry, DLQ & idempotency: retry strategies, poison messages, dead-letter queues, backoff, and the idempotent-consumer/deduplication topic (curriculum-map row 13) in full production depth — idempotency keys, a real `processed_events` table with a unique constraint (the production successor to WP-06's file-backed check), and the crash scenarios (redelivery after a crash before offset commit, an orphaned in-progress claim) that make them necessary. Reuses the WP-07 cluster plus, for its Postgres instance only, `platform/kafka-connect/`. (`lab-12-retry-dlq-idempotency`) | Current |
| WP-14 | Kafka Streams: stream processing, state stores, joins, windows, `TopologyTestDriver`, exactly-once processing. (`lab-17-kafka-streams`) | Next |
