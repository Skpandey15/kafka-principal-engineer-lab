...
| WP-17 | Performance & capacity engineering: a real, hand-built producer benchmark measuring batching and compression's real, measured effect (not simulated numbers), partition count as a real ceiling on both throughput and consumer parallelism, the `fetch.min.bytes`/`fetch.max.wait.ms` latency trade-off, and a worked capacity workbook (`CapacityCalculator`) as real, tested arithmetic that takes a MEASURED per-partition throughput ceiling as its input — tuning and sizing taught together, not as separate tracks. (`lab-16-performance-capacity`) | Current |
| WP-18 | Kafka security: TLS, SASL/SCRAM, ACLs, authentication, authorization, secrets, production security architecture (local-development-only shortcuts clearly marked). (`lab-22-security`) | Next |
