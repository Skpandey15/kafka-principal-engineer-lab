...
| 13 | Idempotent consumer & business exactly-once | Kafka delivery guarantees ≠ business exactly-once processing — where is *your* idempotency boundary? | `docs/retry-dlq/` | Introduced early, at the level of a single `eventId` check against a durable store, in `lab-05` (WP-06); full production treatment (a real `processed_events` table, unique constraints, crash scenarios) alongside retry/DLQ in `lab-12` (WP-13) |
| 14 | Kafka Streams | How do you build stateful stream processing on top of Kafka? | `docs/kafka-streams/` | `lab-13` (WP-14) |
| 15 | Spring Kafka | How does a production framework map onto the primitives you already know? | `docs/spring-kafka/` | `lab-14` (WP-15) |
...
| 18 | Observability | What do you monitor, and what does each signal mean? | `docs/observability/` | `lab-15` (WP-16) |
| 19 | Performance engineering | What actually limits throughput and latency? | `docs/performance/` | `lab-16` (WP-17) |
| 20 | Capacity planning | How do you size a cluster from a workload description? | `docs/performance/` | `lab-16` (WP-17) |
| 21 | Security | How do you secure a cluster without breaking it? | `docs/security/` | `lab-17` (WP-18) |
...
| 23 | Troubleshooting | Given symptoms and metrics, what is the diagnostic sequence? | `docs/observability/` (Section 10 — merged into WP-16 rather than a separate `docs/troubleshooting/`) | all failure labs, diagnostic-sequence framing in `lab-15` (WP-16) |
...
| 26 | Multi-cluster & disaster recovery | Active/passive vs. active/active, RPO/RTO, producer/consumer failover — architecture first, product choice second | [`docs/multi-cluster/KAFKA_MULTI_CLUSTER_AND_DR.md`](../multi-cluster/KAFKA_MULTI_CLUSTER_AND_DR.md) | WP-20 (`lab-19-multi-cluster-dr`) — Demonstrated (real MirrorMaker 2 replication and DR offset translation); active/active covered conceptually only, see the doc's Section 2 |
...
| WP-13 | Retry, DLQ & idempotency: retry strategies, poison messages, dead-letter queues, backoff, and the idempotent-consumer/deduplication topic (curriculum-map row 13) in full production depth — idempotency keys, a real `processed_events` table with a unique constraint (the production successor to WP-06's file-backed check), and the crash scenarios (redelivery after a crash before offset commit, an orphaned in-progress claim) that make them necessary. Reuses the WP-07 cluster plus, for its Postgres instance only, `platform/kafka-connect/`. (`lab-12-retry-dlq-idempotency`) | Done |
| WP-14 | Kafka Streams: stream processing, state stores (materialized, changelog-backed), tumbling windows, stream-table joins, `TopologyTestDriver`, and real multi-instance failure behavior (task migration + changelog restoration when a Streams process dies, standby replicas reducing restoration time — both dedicated failure-matrix rows for this WP), plus exactly-once processing. Reuses the WP-07 cluster; no new platform infrastructure (Streams is a client library). (`lab-13-kafka-streams`) | Done |
| WP-15 | Spring Kafka: listener containers and acknowledgment modes, error handling and dead-letter publishing, non-blocking retry topics (contrasted against WP-13's own blocking approach), and declarative transactions — every Spring abstraction mapped explicitly onto the native Kafka mechanism it wraps (`CONTRIBUTING.md` §18), plus real Spring Boot 4.x findings (autoconfiguration split into `spring-boot-kafka`, custom-bean-suppresses-default behavior). The first Spring Boot lab in the repository; tested via `@EmbeddedKafka`. (`lab-14-spring-kafka`) | Done |
| WP-16 | Kafka observability & troubleshooting: a real JMX exporter → Prometheus → Grafana pipeline (each WP-07 broker now runs a real Prometheus JMX exporter agent), consumer lag computed two ways (natively via the Admin API and via `kafka-exporter`, cross-checked against each other), hot-partition diagnosis, production dashboards, and the operational diagnostic sequence (curriculum-map row 23) — plus this WP's dedicated failure-matrix row (sustained consumer lag with no producer backpressure) and its stated consequence (real data loss once retention exceeds the lag). Adds `platform/observability/`; tested against the real, already-running platform rather than Testcontainers, a deliberate departure explained in the lab README. (`lab-15-observability`) | Done |
| WP-17 | Performance & capacity engineering: a real, hand-built producer benchmark measuring batching and compression's real, measured effect (not simulated numbers), partition count as a real ceiling on both throughput and consumer parallelism, the `fetch.min.bytes`/`fetch.max.wait.ms` latency trade-off, and a worked capacity workbook (`CapacityCalculator`) as real, tested arithmetic that takes a MEASURED per-partition throughput ceiling as its input — tuning and sizing taught together, not as separate tracks. (`lab-16-performance-capacity`) | Done |
| WP-18 | Kafka security: a real, single-node broker secured with TLS (a real self-signed CA and CA-signed cert with real SANs) and SASL/SCRAM-SHA-512 authentication, plus real `StandardAuthorizer` ACL enforcement (real deny, grant, and revoke) — local-development-only shortcuts (the self-signed CA, checked-in lab credentials, `ssl.client.auth=none`) explicitly marked throughout. Adds `platform/kafka-security/`; tested against the real, already-running platform. (`lab-17-security`) | Done |
| WP-19 | Failure engineering & production simulation: disk pressure (the one failure-matrix row explicitly assigned to this WP), simulated safely via a real, size-capped filesystem — plus a production-simulation scenario combining TWO already-demonstrated failure mechanics (a real broker kill, WP-07, and a real consumer-group rebalance, WP-05) deliberately overlapping in ONE running system, proving each WP's own guarantee still holds under simultaneous stress. Every OTHER failure this WP's own name lists is already `Demonstrated` in an earlier WP — see the conceptual doc's own scoping note. (`lab-18-failure-engineering`) | Done |
| WP-20 | Multi-cluster, DR & Principal Engineer capstone: multi-cluster architecture, MirrorMaker 2, disaster recovery, RPO/RTO, active-active vs. active-passive, partition lifecycle engineering, cluster rebalancing and Cruise Control, platform governance, cost engineering, Kafka on Kubernetes/Strimzi, system design labs, the ADR set, the Principal Engineer architecture-decision framework, the source-code reading guide, interview question banks, and milestone assessments — architecture decisions and trade-offs across capacity, security, and reliability, culminating in final production-grade system design. This is the consolidation of every previously-separate Level 5/6 capstone topic into one substantial, final work package. Builds exactly one piece of new real infrastructure (`lab-19-multi-cluster-dr`, real MirrorMaker 2 replication and DR offset translation) and covers the rest conceptually — see `docs/multi-cluster/KAFKA_MULTI_CLUSTER_AND_DR.md`, Section 1, for the full scoping rationale. (`lab-19-multi-cluster-dr`, `system-design/`, `adrs/`, `docs/principal-engineer/`) | Current |
