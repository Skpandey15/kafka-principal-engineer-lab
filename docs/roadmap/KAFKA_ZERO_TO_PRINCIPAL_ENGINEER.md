...
| 6 | KRaft | How does the cluster agree on metadata without ZooKeeper? | `docs/kraft/` | Lab number to be assigned when WP-08 is scheduled |
...
| WP-07 | Replication, ISR & broker failure: replication factor, leaders/followers, ISR, leader election, `acks`, `min.insync.replicas`, broker failure, broker recovery. Real 3-node KRaft cluster (`platform/kafka-cluster/`). (`lab-06-replication-isr-broker-failure`) | Current |
| WP-08 | KRaft controller quorum & failure: controller quorum, metadata quorum, controller failure, leader election, quorum behavior. Lab number to be assigned when scheduled. | Next |
